...
public class VerificacionUpdateBooking implements Question<Boolean> {
    private static final Logger LOGGER = LogManager.getLogger(VerificacionUpdateBooking.class.getName());

    @Override
    public Boolean answeredBy(Actor actor) {
        Gson gson = new Gson();
        LOGGER.info("Verificacion de la actualizacion de la reserva");
        limpiarReporte();
        ...
        Serenity.recordReportData().withTitle("Verificacion de la actualizacion de la reserva").andContents(Serenity.sessionVariableCalled(INFORME_SERENITY));

        return resultadoFinal;
    }

    public static Question<Boolean> contraElResponse() {
        return new VerificacionUpdateBooking();
    }
}
